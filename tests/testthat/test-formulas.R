...
if (doSIRIUS)
{
    anPL <- annotatedPeakList(fCons, precursor = "C9H7NO", groupName = groupNames(fCons)[4], MSPeakLists = plists)
    anPLOnly <- annotatedPeakList(fCons, precursor = "C9H7NO", groupName = groupNames(fCons)[4],
...
plotPrec <- formsGFWithMSMS[[1]][["neutral_formula"]][1]
